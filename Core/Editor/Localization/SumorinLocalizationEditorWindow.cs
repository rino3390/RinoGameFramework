...
namespace Sumorin.GameFramework.Localization.Editor
{
    ...
    /// </summary>
    public class SumorinLocalizationEditorWindow : OdinLocalizationEditorWindow
    {
        private const float ExtendedToolbarHeight = 24f;

        [MenuItem("Tools/Sumorin/Localization Editor", priority = 10_101)]
        public new static void OpenFromMenu()
        {
            var wnd = GetWindow<SumorinLocalizationEditorWindow>();
            wnd.titleContent = new GUIContent("Sumorin Localization");
            wnd.MenuWidth = 300.0f;
        }

        private object sumorinLastSelection;
        ...
        protected override OdinMenuTree BuildMenuTree()
        {
            ...
            tree.Selection.SelectionChanged += type =>
            {
                switch (type)
                {
                    case SelectionChangedType.ItemAdded:
                        if (sumorinLastSelection != null)
                        {
                            switch (sumorinLastSelection)
                            {
                        ...
                        sumorinLastSelection = MenuTree.Selection.SelectedValue;
                        break;
                }
            };
            ...
            foreach (var guid in collectionGUIDs)
            {
                ...
                {
                    var guiCollection = new SumorinStringTableCollectionEditor(stringTableCollection, this, State);
        ...
        protected override void OnImGUI()
        {
            ...
            {
                if (MenuTree?.Selection?.SelectedValue is SumorinStringTableCollectionEditor editor)
                {
        ...
        private void HandleKeyboardShortcuts()
        {
            ...
            if (!(MenuTree?.Selection?.SelectedValue is SumorinStringTableCollectionEditor ctrlEditor))
                return;
        ...
        private void DrawExtendedToolbarButtons()
        {
            ...
            {
                if (editor is SumorinStringTableCollectionEditor sumorinEditor)
                {
                    sumorinEditor.DuplicateSelectedEntry();
                }
            }
        ...
        private Vector2 CalculateTargetCellPosition(SumorinStringTableCollectionEditor editor)
        {
            // OdinLocalizationConstants 值
            const float originalToolbarHeight = 28f;
            const float columnHeight = 38f;
